...
properties (Constant)
  % Constant properties of trmodel
end
...
methods (Static)
  ...
  function [p_fuel_notax, pnew_notax, pscrap_notax] = price_notax(mp)
    ...
    for car=1:mp.ncartypes 
      ...
      % new car prices before registration taxes
      % 1. high bracket 
      cutoff = mp.K_cartax_hi*(1+mp.cartax_lo);
      high = max(0, (mp.pnew{car} / (1+mp.vat) - cutoff)/(1+mp.cartax_hi)); 
      
      % 2. low bracket
      if mp.pnew{car}/(1+mp.vat)>=cutoff
          low = cutoff/(1+mp.cartax_lo); 
      else
          low = (mp.pnew{car} / (1+mp.vat))/(1+mp.cartax_lo); 
      end
      
      % 3. total
      pnew_notax{car} = high + low; 
      
      assert(low>=0 && high>=0, 'Negative tax contributions');

      % scrap price before tax
      % scrap subsidies or other policy parameters affecting pscrap may be added here
     if nargout >=3 
  ...
  function [p_fuel, pnew, pscrap] = price_aftertax(mp)
    ...
    % This function computes prices after taxes given before tax prices and tax rates in mp
    % When implemneting new taxes/subisdies remember to update the correspoding after tax function below. 

    p_fuel  = mp.p_fuel_notax*(1+mp.tax_fuel);
          
    pnew = cell(1, mp.ncartypes); 
    for car=1:mp.ncartypes

      % 1. low part: caps off at K
      low  = min(mp.K_cartax_hi, mp.pnew_notax{car})   * (1+mp.cartax_lo); 
      
      % 2. high part: only on the part above K (and never
      % negative)
      high = max(0, mp.pnew_notax{car}-mp.K_cartax_hi) * (1+mp.cartax_hi); 
      
      % 3. add VAT 
      pnew{car} = (1+mp.vat)*(low + high); 
      
      assert(low>=0 && high>=0, 'Negative contributions in tax rates should not happen. Aborting.'); 

      % scrap price before tax
      % scrap subsidies or other policy parameters affecting pscrap may be added here
      pscrap{car}=mp.pscrap_notax{car};
    end
  end
