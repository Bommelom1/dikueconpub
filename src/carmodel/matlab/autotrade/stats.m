classdef stats
    ...
    properties (Constant)
        ...
        extern_cost_per_km =          ...
            ...
            + .044565*(1-stats.gas_share); % air pollution, diesel
    ...
    methods (Static)
        ...
        function out = compute_outcomes(mp, s, sol)
            ...
            % compute tax revenue
            [out.revenue_car_tau, out.revenue_fuel_tau] = stats.revenue(mp, s, sol);
            ...
            % total net welfare
            out.social_surplus_total = out.consumer_surplus + out.total_revenue - out.total_extern;
            out.social_surplus_ex_co2 = out.consumer_surplus + out.total_revenue - out.extern_vkt; % simply excludes the monetized value of CO2 emissions
            out.social_surplus_total_highco2 = out.consumer_surplus + out.total_revenue - out.extern_vkt - out.total_co2 * stats.social_cost_carbon_dkk_high;  % computes the social cost of CO2 at a higher price
        end
        ...
        function [revenue_car_tau, revenue_fuel_tau] = revenue(mp, s, sol)
            ...
            % New car sales tax
            for j=1:mp.ncartypes
                tax_car=mp.pnew{j}-mp.pnew_notax{j}; %tax per new car
                for tau=1:mp.ntypes
